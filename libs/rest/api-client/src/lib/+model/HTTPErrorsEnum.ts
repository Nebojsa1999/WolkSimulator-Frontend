export enum HTTP_ERRORS {
   ...
   * See Basic access authentication and Digest access authentication. 401 semantically means
   ...
   * of some form of digital cash or micro payment scheme, but that has not happened,
   * and this code is not usually used.
   * Google Developers API uses this status if a particular developer has exceeded
   ...
   * This should be used when a resource has been intentionally removed and the resource
   * should be purged.
   ...
   * Hyper Text Coffee Pot Control Protocol, and is not expected to be implemented by
   * actual HTTP servers.
...
export function getHttpErrorType(value: number): string | undefined {
  return Object.keys(HTTP_ERRORS).find((key: string) => HTTP_ERRORS[key as keyof typeof HTTP_ERRORS] === value);
}
